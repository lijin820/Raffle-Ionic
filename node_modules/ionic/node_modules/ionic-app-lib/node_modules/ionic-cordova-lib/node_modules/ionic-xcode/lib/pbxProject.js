...
pbxProject.prototype.parse = function (cb, options) {
    // Changing fork to be spawn - the reasoning, we are relying on 
    // electron, and as such, fork is handled internally from electron without stdio.
    // As such, we switch to spawn, hook into its sdtio streams, and rely on data
    // that we piece together from the buffer to complete the data returned,
    // json parse it, and return that on close event.
    var worker = spawn(__dirname + '/parseJob.js', [this.filepath], options);
    var me = this;
    me.capturedStdout = '';
    ...
    if (worker.stdout) {
      ...
      worker.stdout.on('data', function(data) {
        me.capturedStdout += data;
      });

      worker.on('close', function(endData) {
        me.hash = JSON.parse(me.capturedStdout);
        me.emit('end', null, me.hash);
      });
    ...
    // This is mainy unused since the switch to spawn.
